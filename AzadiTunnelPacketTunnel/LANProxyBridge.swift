...
final class LANProxyBridge: @unchecked Sendable {
    ...
    private struct ListenerResult {
        ...
        /// Technical bind interface: Wi-Fi IP or "0.0.0.0" when we fell back to any-interface.
    ...
    /// Refreshes Psiphon's local proxy ports without tearing down active LAN listeners.
    /// New connections use the latest endpoint snapshot; existing relays finish normally.
    func updateUpstream(_ upstream: Endpoints) {
        configLock.lock()
        if let current = config {
    ...
    private func setConfiguration(_ value: Configuration?) {
        configLock.lock()
        config = value
        configLock.unlock()
    }
    ...
    private func handleHttpAccept(_ incoming: NWConnection) {
        ...
        if SharedSettingsStore.shared.tunnelEffectiveAppSettings.proxyOnlyModeEnabled {
    ...
    private func dialViaPsiphonSOCKS(
        ...
        // Domain targets are sent to Psiphon's SOCKS listener as domain names (or a fixed
        // provider bootstrap IP). This keeps proxy-host resolution inside the established tunnel
        // and avoids recursively invoking the loopback system proxy during startup.
        let dialHost = Socks5TCPClient.socksTargetHost(for: host)
        if dialHost != host {
            SharedLogger.shared.logRaw(
