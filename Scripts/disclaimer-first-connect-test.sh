...
LOG_OUT="${ROOT}/Tooling/test-logs/disclaimer-first-connect-${STAMP}.log"

mkdir -p "${ROOT}/Tooling/test-logs"

echo "=== disclaimer-first-connect-test.sh ===" | tee "${LOG_OUT}"
...
xcodebuild test -project "${ROOT}/AzadiTunnel.xcodeproj" -scheme AzadiTunnelUITests \
  ...
  -only-testing:AzadiTunnelUITests/AzadiTunnelLegalTests/testDisclaimerBlocksUntilAccepted \
  2>&1 | tee -a "${LOG_OUT}"
